...
export class FactureReturnComponent implements OnInit {
  ...
  getFactureDetails(FactureID) {
    this.factureReturnService.getFactureDetails(FactureID, "FR").subscribe(
      Response => {
        this.factureHeader = Response[0];
        this.invoiceForm.get("invID").setValue(FactureID);
        this.invoiceForm
          .get("invoiceDate")
          .setValue(this.factureHeader[0].inv_date_req);
        this.invoiceForm
          .get("clientName")
  ...
  deleteItem(i, id, itemIsDamaged) {
    ...
    var index = FactureReturnComponent.findWithAttr(
      FactureReturnComponent.selectedItems,
      "id",
      "gate",
      id.value,
  ...
  editReturnInvoice() {
    this.factureReturnService
      .editReturnInvoice(this.invoiceForm.value)
      .subscribe(
        Response => {
          swal({
            type: "success",
  ...
  addItemsToFacture() {
    ...
    FactureReturnComponent.selectedItems.forEach(element => {
      ...
      var i = FactureReturnComponent.findWithAttr(
      ...
      if (this.itemsEditForm.length == 0) {
        if (i == -1) this.addRow(element);
      } else if (this.itemsEditForm.length != 0) {
        var i2 = FactureReturnComponent.findWithAttr2(
          this.itemsEditForm.value,
          "itemID",
          "isDamaged",
          ID,
          isDamaged
  ...
  openMultiSelect(mutliSelectModal) {
    this.modalReference = this.modalService.open(mutliSelectModal, {
      centered: true,
      size: "lg",
    ...
    var multiSelectDT = $("#stockDT").DataTable({
      ...
      ajax: {
        type: "get",
        url:
          "http://localhost/DMD-Inventory/src/assets/api/dataTables/multiselectDT-Gate.php",
        data: {},
        cache: true,
        async: true
      },
    ...
    multiSelectDT.on("select", function (e, dt, type, indexes) {
      ...
      rows.forEach(element => {
        ...
        if (
          FactureReturnComponent.findWithAttr(
            FactureReturnComponent.selectedItems,
            "id",
            "isDamaged",
            ID,
          ...
          FactureReturnComponent.selectedItems.push({
            id: ID,
            name: name,
            isDamaged: isDamaged,
            colisage: colisage
