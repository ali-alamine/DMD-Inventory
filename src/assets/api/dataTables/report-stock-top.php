...
$rowsCount = mysqli_fetch_assoc(mysqli_query(openConn(), "SELECT 10 as exp"))['exp'];
...
if (isset($_GET["search"]["value"]) && !empty($_GET["search"]["value"])) {
    ...
    $getAllFactureQuery = "select * from person inner join ( select count(*) as invoiceCount ,inv_perID from invoice where inv_type = 'FC' GROUP by `inv_perID` order by invoiceCount DESC limit 10) as d on person.perID = d.`inv_perID` " . $orderString . " LIMIT " . $rowsReq . " OFFSET " . $start;
