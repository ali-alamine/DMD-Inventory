...
if (isset($_GET["search"]["value"]) && !empty($_GET["search"]["value"])) {
    ...
    $getAllFactureQuery = "select * from item  where is_client =1 AND  pid != 1 and (name like '%" . $search . "%' OR phone like '%" . $search . "%' OR address like '%" . $search . "%' ) " . $orderString . " LIMIT " . $rowsReq . " OFFSET " . $start;
